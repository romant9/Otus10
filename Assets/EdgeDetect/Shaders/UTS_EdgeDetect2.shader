...
//Definitions of Structure, vertex shader and fragement shader fot Sobel Depth and Sobel Depth Thin
	...
	{	
		...
		//Original
		//Col = _EdgesColor * (1.0 - Sobel) + Sobel;
		//return Col * lerp(tex2D(_MainTex, i.uv[0].xy), _TestColor, _BgFade);
		
		//My
		float4 fillColor = float4(lerp(Col.rgb, float3(1, 1, 1), (1 - Sobel.xxx) * _EdgesColor.a), Col.a);
		float4 finalColor = float4(lerp(fillColor.rgb, _EdgesColor.rgb, (1 - Sobel.xxx)* _EdgesColor.a), Col.a); 
		return finalColor;
	...
	{	
		...
		depthsDiag = (depthsDiag > centerDepth.xxxx*2) ? depthsDiag : centerDepth.xxxx;
		depthsAxis = (depthsAxis > centerDepth.xxxx*2) ? depthsAxis : centerDepth.xxxx;
		...
		float4 SobelH = depthsDiag * HorizDiagCoeff + depthsAxis * HorizAxisCoeff*10;
		float4 SobelV = depthsDiag * VertDiagCoeff + depthsAxis * VertAxisCoeff*10;
		...
		float4 finalColor = Col * lerp(tex2D(_MainTex, i.uv[0].xy), _TestColor, _BgFade);*/
	

		//My
		float3 hsv = HsvToRgb(_EdgesColor.rgb);
		//float3 _EdgesColorV = lerp(_EdgesColor.rgb * 1.5, Col.rgb - _EdgesColor.rgb , (1-hsv.z));
		float3 _EdgesColorV = lerp(_EdgesColor.rgb * 1.5, Col.rgb * 0.5, (1 - hsv.z));
		float4 fillColor = float4(lerp(Col.rgb, float3(1, 1, 1), (1 - Sobel.xxx) * _EdgesColor.a), Col.a);
		float4 finalColor = float4(lerp(fillColor.rgb, _EdgesColorV.rgb, (1 - Sobel.xxx) * _EdgesColor.a), Col.a);

		//return float4(finalColor.rgb, Col.a);
		return finalColor;
	}
...
//Definitions of structure, vertex shader and fragment shader for Roberts Cross Depth Normals
	...
	inline float CheckSame (float2 centerNormal, float centerDepth, float4 theSample)
	{
		...
		//int isSameNormal = (diff.x + diff.y) * _Sensitivity.y < 3*isSameDepth;
	...
	v2f vertRobert( appdata_img v ) 
	{
		...
		o.uv[1] = uv + _MainTex_TexelSize.xy * float2(0.5, 0.5) * _SampleDistance;
		o.uv[2] = uv + _MainTex_TexelSize.xy * float2(-0.5,-0.5) * _SampleDistance;
		o.uv[3] = uv + _MainTex_TexelSize.xy * float2(-0.5, 0.5) * _SampleDistance;
		o.uv[4] = uv + _MainTex_TexelSize.xy * float2(0.5,-0.5) * _SampleDistance;
		return o;
	} 
	...
	half4 fragRobert(v2f i) : SV_Target {				
		...
		//edge *= CheckSame(sample2.xy, DecodeFloatRG(sample2.zw), sample3);
		edge *= CheckSame(sample3.xy, DecodeFloatRG(sample3.zw), sample4);
		//edge *= CheckSame(sample4.xy, DecodeFloatRG(sample4.zw), sample1);
		

		//NK
		float4 Col = tex2D(_MainTex, i.uv[0].xy);
		//float4 mix = (_EdgesColor * Col * (1.0 - edge) + edge) * lerp(tex2D(_MainTex, i.uv[0].xy), _TestColor, _BgFade);
		//float4 final = float4(lerp(mix.rgb, Col.rgb, 1 - _EdgesColor.a), Col.a);
		//float4 mix =  Col + _EdgesColor*(1-edge)*Col;
		//float4 mix = float4(1,1,1,1)*(1-edge)* _EdgesColor;
		float4 mix = edge * Col + _EdgesColor * (1.0 - edge);
		float4 final = float4(lerp(Col.rgb, mix.rgb, _EdgesColor.a), Col.a);
		return final;
		//return Col * lerp(tex2D(_MainTex, i.uv[0].xy), _TestColor, _BgFade);
